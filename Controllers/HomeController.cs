...
ViewBag.usuario = usuarioCompleto;
       ...
       ViewBag.tipoUsuario = usuarioCompleto.tipoUsuario;
...
public IActionResult JuegoOrdenarPictogramas(int? index, bool? continuar, string? MensajeError)
{
    string? usuarioJson = HttpContext.Session.GetString("Usuario"); // Crear el usuario session
        if (string.IsNullOrEmpty(usuarioJson))
        {
...
public IActionResult VerificarRespuesta(string opcion)
{
    ...
    {
       string mensajeEror = "Intenta de nuevo";
       
        return RedirectToAction("JuegoOrdenarPictogramas", new { index = idPreguntaActual, MensajeError = mensajeEror});
    }
}
